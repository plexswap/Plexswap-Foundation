import { AutoRow, Box, Button, Card, CardBody, Flex, Heading, Text, useMatchBreakpoints } from '@plexswap/ui-plex'
...
export default function PoolCorePage() {
  ...
  const [, pair] = useCorePair(baseCurrency ?? undefined, currencyB ?? undefined)
  ...
  const { data: isFarmExistActiveForPair } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (farmPair) {
        const poolInfo = await ChiefFarmerContract?.read.poolInfo([BigInt(farmPair.pid)])
    ...
    enabled: Boolean(chainId && pair && ChiefFarmerContract),
      ...
      <BodyWrapper>
        <AppHeader
          ...
          buttons={
            !isMobile && (
              <>
                <NextLinkFromReactRouter to={`/core/add/${pair?.token0.address}/${pair?.token1.address}?increase=1`}>
                  <Button width="100%" disabled={!pair}>
                    {t('Add')}
                  </Button>
                </NextLinkFromReactRouter>
                <NextLinkFromReactRouter
                  to={`/core/remove/${pair?.token0.address}/${pair?.token1.address}`}
                ...
                {isFarmExistActiveForPair === 'notexist' && (
                  <NextLinkFromReactRouter to={`/core/migrate/${pair?.liquidityToken?.address}`}>
                    <Button variant="secondary" width="100%" disabled={!pair}>
                      {t('Migrate')}
                    </Button>
                  </NextLinkFromReactRouter>
        ...
        <CardBody>
          {isMobile && (
            <>
              ...
              <NextLinkFromReactRouter to={`/core/remove/${pair?.token0.address}/${pair?.token1.address}`}>
...
PoolCorePage.chains = CHAIN_IDS
PoolCorePage.screen = true

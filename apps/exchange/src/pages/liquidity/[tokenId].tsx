...
import { isActiveExtendedFarm } from '@plexswap/farms'
import { Currency, CurrencyAmount, Fraction, Percent, Price, Token } from '@plexswap/sdk-core'
import {
    AtomBox,
    AutoColumn,
    AutoRow,
...
import { ConfirmationModalContent, NextLinkFromReactRouter } from '@plexswap/widgets-internal'

import { Trans, useTranslation } from '@plexswap/localization'
import { ChiefFarmerExtended, NonfungiblePositionManager, Pool, Position, isPoolTickInRange } from '@plexswap/sdk-extended'
...
import { useExtendedPositionFees } from 'hooks/extended/useExtendedPositionFees'
import { useExtendedPositionFromTokenId, useExtendedTokenIdsByAccount } from 'hooks/extended/useExtendedPositions'
...
import getPriceOrderingFromPositionForUI from 'hooks/extended/utils/getPriceOrderingFromPositionForUI'
...
import { AprCalculator } from 'views/AddLiquidityExtended/components/AprCalculator'
import RateToggle from 'views/AddLiquidityExtended/formViews/ExtendedFormView/components/RateToggle'
...
export default function PoolPage() {
  ...
  const { loading, position: positionDetails } = useExtendedPositionFromTokenId(parsedTokenId)
  ...
  const hasActiveFarm = useMemo(
    () => farmDetail && isActiveExtendedFarm(farmDetail.farm, farmDetail.poolLength),
    [farmDetail],
  )
  ...
  // fees
  const [feeValue0, feeValue1] = useExtendedPositionFees(pool ?? undefined, positionDetails?.tokenId, receiveWNATIVE)
  ...
  const { tokenIds: stakedTokenIds, loading: tokenIdsInCFextendedLoading } = useExtendedTokenIdsByAccount(
    chieffarmerExtended?.address,
    account,
  )

  const isStakedInCFExtended = tokenId && Boolean(stakedTokenIds.find((id) => id === tokenId))

  const manager = isStakedInCFExtended ? chieffarmerExtended : positionManager
  const interfaceManager = isStakedInCFExtended ? ChiefFarmerExtended : NonfungiblePositionManager
  ...
  const isOwnNFT = isStakedInCFExtended || ownsNFT
  ...
  const farmingTips =
    inRange && ownsNFT && hasActiveFarm && !isStakedInCFExtended ? (
      <Message variant="primary" mb="2em">
        <Box>
          <Text display="inline" bold mr="0.25em">{`${currencyQuote?.symbol}-${currencyBase?.symbol}`}</Text>
          <Text display="inline">
            {t(
              'has an active Plexswap farm. Stake your position in the farm to start earning with the indicated APR with WAYA farming.',
            )}
          </Text>
  ...
  return (
    <Page>
      {!isLoading && <NextSeo title={`${currencyQuote?.symbol}-${currencyBase?.symbol} Extended LP #${tokenIdFromUrl}`} />}
      <BodyWrapper>
        {isLoading ? (
                ...
                <Box mb={['8px', '8px', 0]} width="100%" style={{ flex: 1 }} minWidth={['auto', 'auto', 'max-content']}>
                  ...
                  <RowBetween gap="16px" flexWrap="nowrap">
                    <Text fontSize="14px" color="textSubtle" style={{ wordBreak: 'break-word' }}>
                      Extended LP #{tokenIdFromUrl} / {new Percent(feeAmount || 0, 1_000_000).toSignificant()}%{' '}
                      {t('fee tier')}
                    </Text>
                    {isMobile && (
                      <Flex>
...
function PositionHistory_({
  ...
  const client = extendedClients[chainId as ChainId]

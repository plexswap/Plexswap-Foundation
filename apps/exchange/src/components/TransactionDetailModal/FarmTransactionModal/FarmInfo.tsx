...
import { Box, Flex, LightBulbIcon, StyledLink, Text, useTooltip } from '@plexswap/ui-plex'
...
const FarmPending: React.FC<React.PropsWithChildren<FarmInfoProps>> = ({ pickedData }) => {
  ...
  const { amount, lpSymbol, type } = pickedData.specialFarm
  const title = type === SpecialFarmStepType.STAKE ? t('Staking') : t('Unstaking')
...
const FarmResult: React.FC<React.PropsWithChildren<FarmInfoProps>> = ({ pickedData }) => {
  const { t } = useTranslation()
  const { amount, lpSymbol, type, steps } = pickedData?.specialFarm ?? {}
  const firstStep = steps?.find((step) => step.step === 1)
  const text =
    type === SpecialFarmStepType.STAKE ? t('token have been staked in the Farm!') : t('token have been unstaked!')
...
const FarmError: React.FC<React.PropsWithChildren<FarmInfoProps>> = ({ pickedData }) => {
  ...
  const { amount, lpSymbol, type, steps } = pickedData.specialFarm
  const text = type === SpecialFarmStepType.STAKE ? t('The attempt to stake') : t('The attempt to unstake')
  const errorText = type === SpecialFarmStepType.STAKE ? t('Token fail to stake.') : t('Token fail to unstake.')
...
const FarmInfo: React.FC<React.PropsWithChildren<FarmInfoProps>> = ({ pickedData }) => {
  const { status } = pickedData?.specialFarm ?? {}

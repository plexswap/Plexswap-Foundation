...
import useTokenBalance, { useBSCWayaBalance } from 'hooks/useTokenBalance'
import { ChainLogo } from 'components/Logo/ChainLogo'
import { getBlockExploreLink, getBlockExploreName } from 'utils'
import { getFullDisplayBalance, formatBigInt } from '@plexswap/utils/formatBalance'
...
const WalletInfo: React.FC<WalletInfoProps> = ({ hasLowNativeBalance, onDismiss }) => {
  ...
  const { balance: wayaBalance, fetchStatus: wayaFetchStatus } = useBSCWayaBalance()
  ...
  const showNativeEntryPoint = Number(nativeBalance?.data?.value) === 0 && SUPPORT_BUY_CRYPTO.includes(chainId as ChainId)
  ...
  return (
    <>
            ...
            {!nativeBalance.isFetched ? (
              ...
              <Flex>
                ...
                {showNativeEntryPoint ? (
                  ...
                  >
                    <InfoFilledIcon pl="2px" fill="#000" color="#f5a500" width="22px" />
                  </TooltipText>
                ) : null}
      ...
      <Box mb="24px">
        ...
        <Flex alignItems="center" justifyContent="space-between">
          <Text color="textSubtle">{t('WAYA Balance')}</Text>
          {wayaFetchStatus !== FetchStatus.Fetched ? (
            <Skeleton height="22px" width="60px" />
          ) : (
            <Text>{formatBigInt(wayaBalance, 3)}</Text>
          )}
        </Flex>
      </Box>

import { BlockIcon, CheckmarkCircleIcon, Flex, BscScanIcon, RefreshIcon } from '@plexswap/ui-plex'
...
const renderIcon = (txn: TransactionDetails) => {
  const { receipt, specialFarm } = txn
  if (!txn.receipt || specialFarm?.status === FarmTransactionStatus.PENDING) {
    return <RefreshIcon spin width="24px" />
  }

  const isFarmStatusSuccess = specialFarm ? specialFarm.status === FarmTransactionStatus.SUCCESS : true

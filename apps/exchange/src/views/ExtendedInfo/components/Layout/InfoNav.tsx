      ...
      <Flex>
        <Box>
          <ButtonMenu activeIndex={activeIndex} scale="sm" variant="subtle">
            <ButtonMenuItem as={NextLinkFromReactRouter} to={`/${extendedInfoPath}${chainPath}${stableSwapQuery}`}>
              {t('Overview')}
            </ButtonMenuItem>
            <ButtonMenuItem as={NextLinkFromReactRouter} to={`/${extendedInfoPath}${chainPath}/pairs${stableSwapQuery}`}>
              {t('Pairs')}
            </ButtonMenuItem>
            <ButtonMenuItem as={NextLinkFromReactRouter} to={`/${extendedInfoPath}${chainPath}/tokens${stableSwapQuery}`}>
              {t('Tokens')}
            </ButtonMenuItem>
          </ButtonMenu>
        </Box>
...
export const NetworkSwitcher: React.FC<{ activeIndex: number }> = ({ activeIndex }) => {
  ...
  const switchNetwork = useCallback(
    (chianId: number) => {
      ...
      if (activeIndex === 0) router.push(`/${extendedInfoPath}${chainPath}`)
      if (activeIndex === 1) router.push(`/${extendedInfoPath}${chainPath}/pairs`)
      if (activeIndex === 2) router.push(`/${extendedInfoPath}${chainPath}/tokens`)
    },
    [router, activeIndex],
  )

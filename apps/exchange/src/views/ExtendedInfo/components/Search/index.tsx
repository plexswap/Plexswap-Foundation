...
import { Flex, Input, Skeleton, Text, useMatchBreakpoints } from '@plexswap/ui-plex'
          ...
          <Menu ref={menuRef}>
            ...
            {tokensForList.slice(0, tokensShown).map((token) => {
              return (
                <HoverRowLink
                  onClick={() =>
                    handleItemClick(`/${extendedInfoPath}${chainPath}/tokens/${token.address}${stableSwapQuery}`)
                  }
            ...
            {poolForList?.slice(0, poolsShown).map((p) => {
              return (
                <HoverRowLink
                  onClick={() => handleItemClick(`/${extendedInfoPath}${chainPath}/pairs/${p.address}${stableSwapQuery}`)}

...
const GLOBAL_CHART = gql`
  query plexDayDatas($startTime: Int!, $skip: Int!) {
    plexDayDatas(first: 1000, skip: $skip, where: { date_gt: $startTime }, orderBy: date, orderDirection: asc) {
      id
      date
      volumeUSD
      tvlUSD
...
export async function fetchGlobalChartData(
  ...
  try {
    ...
    const shouldUserDerivedData = chainId === ChainId.PLEXCHAIN

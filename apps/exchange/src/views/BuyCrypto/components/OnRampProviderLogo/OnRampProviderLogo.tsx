import type { Token } from '@plexswap/sdk-core'
import { Box, Skeleton, TokenPairImage } from '@plexswap/ui-plex'
      ...
      {mode === 'onramp-fiat' ? (
        ...
        <Box width={`${size}px`} height={`${size}px`}>
          <TokenPairImage
            ...
            secondarySrc={`${ASSET_CDN}/images/chains/${currency.chainId}.png`}
          />
        </Box>
      )}
    </>
...
export const BtcLogo = ({ size = 24 }: { size?: number }) => (
  <Image src={`${ASSET_CDN}/images/on-ramp-providers/btc.svg`} alt="bitcoin-logo" width={size} height={size} />
)

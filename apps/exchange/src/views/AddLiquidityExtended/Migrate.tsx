...
import { CurrencyAmount, ERC20Token, Fraction, NATIVE, Pair, Price, WNATIVE, ZERO } from '@plexswap/sdk-core'
import {
    AtomBox,
    AutoColumn,
    AutoRow,
...
import { useUserSlippagePercent } from '@plexswap/utils/user'
import { FeeAmount, Pool, Position, priceToClosestTick, TickMath } from '@plexswap/sdk-extended'
import { LiquidityChartRangeInput } from '@plexswap/widgets-internal'
...
import { useExtendedMigratorContract, usePairContract } from 'hooks/useContract'
import { useCorePair } from 'hooks/usePairs'
...
import { useDensityChartData } from 'views/AddLiquidityExtended/hooks/useDensityChartData'
...
import { ResponsiveTwoColumns } from 'views/AddLiquidityExtended'
import { useInitialRange } from 'views/AddLiquidityExtended/formViews/ExtendedFormView/form/hooks/useInitialRange'
import FeeSelector from './formViews/ExtendedFormView/components/FeeSelector'
import RangeSelector from './formViews/ExtendedFormView/components/RangeSelector'
import RateToggle from './formViews/ExtendedFormView/components/RateToggle'
import { useExtendedMintActionHandlers } from './formViews/ExtendedFormView/form/hooks/useExtendedMintActionHandlers'
import { useRangeHopCallbacks } from './formViews/ExtendedFormView/form/hooks/useRangeHopCallbacks'
import { useExtendedFormState } from './formViews/ExtendedFormView/form/reducer'
import { HandleFeePoolSelectFn } from './types'

export function Migrate({ corePairAddress }: { corePairAddress: Address }) {
  const pairContract = usePairContract(corePairAddress)
  const { chainId } = useActiveChainId()

  const { data: token0Address } = useReadContract({
    abi: pairContract?.abi,
    address: corePairAddress,
    functionName: 'token0',
    chainId,
  })
  ...
  const [, pair] = useCorePair(token0 ?? undefined, token1 ?? undefined)
  ...
  return (
    <CorePairMigrate
      corePairAddress={corePairAddress}
      token0={token0!}
      token1={token1!}
      pair={pair}
...
function CorePairMigrate({
  ...
  const { balance: pairBalance } = useTokenBalance(corePairAddress)
  ...
  const { onLeftRangeInput, onRightRangeInput, onBothRangeInput } = useExtendedMintActionHandlers(noLiquidity)
  ...
  const coreSpotPrice = useMemo(
    () => new Price(token0, token1, reserve0.quotient, reserve1.quotient),
    [token0, token1, reserve0, reserve1],
  )
  const extendedSpotPrice = pool?.token0Price ?? undefined

  const priceDifferenceFraction: Fraction | undefined = useMemo(() => {
    const result = coreSpotPrice && extendedSpotPrice ? extendedSpotPrice.divide(coreSpotPrice).subtract(1).multiply(100) : undefined
    if (result?.lessThan(ZERO)) {
      return result.multiply(-1)
    }
    return result
  }, [coreSpotPrice, extendedSpotPrice])
  ...
  const tick = useMemo(() => pool?.tickCurrent ?? priceToClosestTick(coreSpotPrice), [pool?.tickCurrent, coreSpotPrice])
  ...
  const migrator = useExtendedMigratorContract()
  const [
    signatureData,
    // setSignatureData
  ] = useState<{
        ...
        return migrator.write
          ...
          .then((response) => {
            addTransaction(
              ...
              {
                type: 'migrate-extended',
                translatableSummary: {
                  text: 'Migrated %symbolA% %symbolB% Core liquidity to Extended',
          ...
          {noLiquidity && (
            <AtomBox>
              <Message variant="warning">
                <MessageText>
                  {t(
                    'You are the first liquidity provider for this Plexswap Extended pool. Your liquidity will migrate at the current V2 price.',
                  )}
                  <MessageText>
                ...
                <AutoColumn gap="sm" style={{ marginTop: '12px' }}>
                  <RowBetween>
                    <Text>
                      ...
                      {invertPrice
                        ? `${coreSpotPrice?.invert()?.toSignificant(6)} ${currency0?.symbol}`
                        : `${coreSpotPrice?.toSignificant(6)} ${currency1?.symbol}`}
                    </Text>
                  </RowBetween>
                </AutoColumn>
              )}
          ...
          {largePriceDifference ? (
            <GreyCard>
              <AutoColumn gap="sm">
                ...
                <RowBetween>
                  <Text fontSize={14}>
                    Extended {invertPrice ? currency1?.symbol : currency0?.symbol} {t('Price')}:
                  </Text>
                  <Text fontSize={14}>
                    {invertPrice
                      ? `${extendedSpotPrice?.invert()?.toSignificant(6)} ${currency0?.symbol}`
                      : `${extendedSpotPrice?.toSignificant(6)} ${currency1?.symbol}`}
                  </Text>
                </RowBetween>
              ...
              <Text fontSize={14} style={{ marginTop: 8, fontWeight: 400 }}>
                {t('You should only deposit liquidity into Plexswap Extended at a price you believe is correct.')} <br />
                {t(
                  'If the price seems incorrect, you can either make a swap to move the price or wait for someone else to do so.',
                )}
              </Text>
            </GreyCard>
          ) : !noLiquidity && extendedSpotPrice ? (
            <RowBetween>
              <Text fontSize={14}>
                Extended {invertPrice ? currency1?.symbol : currency0?.symbol} {t('Price')}:
              </Text>

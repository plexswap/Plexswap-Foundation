...
import { AutoColumn, Box, Button, CardBody, useModal } from '@plexswap/ui-plex'
import { ConfirmationModalContent } from '@plexswap/widgets-internal'

import { useIsExpertMode, useUserSlippage } from '@plexswap/utils/user'
import { ChiefFarmerExtended, FeeAmount, NonfungiblePositionManager } from '@plexswap/sdk-extended'
...
import { ExtendedSubmitButton } from './components/ExtendedSubmitButton'
import LockedDeposit from './formViews/ExtendedFormView/components/LockedDeposit'
import { PositionPreview } from './formViews/ExtendedFormView/components/PositionPreview'
...
export default function IncreaseLiquidityExtended({ currencyA: baseCurrency, currencyB }: AddLiquidityExtendedPropsType) {
  ...
  const { position: existingPositionDetails, loading: positionLoading } = useExtendedPositionFromTokenId(
  ...
  const { onFieldAInput, onFieldBInput } = useExtendedMintActionHandlers(noLiquidity)
  ...
  const isStakedInCFExtended = useMemo(
  ...
  const onIncrease = useCallback(async () => {
    ...
    if (tokenIdsInCFextendedLoading || !positionManager || !baseCurrency || !quoteCurrency) {
      return
    }
      ...
      getViemClients({ chainId })
        ...
        .then((response) => {
          ...
          addTransaction(
            { hash: response },
            {
              type: 'increase-liquidity-extended',

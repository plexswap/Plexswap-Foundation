...
import { Currency, CurrencyAmount, Price, Token, ZERO } from '@plexswap/sdk-core'
import { CalculateIcon, Flex, IconButton, QuestionHelper, RocketIcon, Text, TooltipText } from '@plexswap/ui-plex'
import { BIG_ZERO } from '@plexswap/utils/bigNumber'
import { formatPrice } from '@plexswap/utils/formatFractions'
import { FeeCalculator, Pool, encodeSqrtRatioX96, isPoolTickInRange, parseProtocolFees } from '@plexswap/sdk-extended'
import {
    RoiCalculatorModalCore,
    RoiCalculatorPositionInfo,
    useAmountsByUsdValue,
    useRoi,
} from '@plexswap/widgets-internal/roi'
import { useWayaPrice } from 'hooks/useWayaPrice'
import { useRouter } from 'next/router'
import { useCallback, useMemo, useState } from 'react'
import { styled } from 'styled-components'

import { PositionDetails, getPositionFarmApr, getPositionFarmAprFactor } from '@plexswap/farms'
...
import { BoostStatus, useBoostStatus } from 'views/Farms/components/YieldBooster/hooks/Extended/useBoostStatus'
import { useUserPositionInfo } from 'views/Farms/components/YieldBooster/hooks/Extended/useWayaExtendedInfo'
...
export function AprCalculator({
  ...
  const { pool, ticks, price, pricesAtTicks, parsedAmounts, currencyBalances } = useExtendedDerivedInfo(
  ...
  const { ticks: data } = useAllExtendedTicks(baseCurrency, quoteCurrency, feeAmount)
  ...
  const { positionFarmApr, positionFarmAprFactor } = useMemo(() => {
    if (!farm || !wayaPrice || !positionLiquidity || !amount0 || !amount1 || !inRange) {
      return {
        positionFarmApr: '0',
        positionFarmAprFactor: BIG_ZERO,
      }
    }
    const { farm: farmDetail, wayaPerSecond } = farm
  ...
  }, [farm, wayaPrice, positionLiquidity, amount0, amount1, inRange])

  // NOTE: Assume no liquidity when opening modal
  const { onFieldAInput, onBothRangeInput, onSetFullRange } = useExtendedMintActionHandlers(false)
  ...
  return (
    <>
      ...
      <RoiCalculatorModalCore
        ...
        wayaAprFactor={positionFarmAprFactor.times(isBoosted ? boostMultiplier : 1)}

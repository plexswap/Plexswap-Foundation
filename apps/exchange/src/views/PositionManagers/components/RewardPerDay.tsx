...
import { Svg, SvgProps, Tag, TagProps } from '@plexswap/ui-plex'
...
export const RewardPerDay: React.FC<{ rewardPerSec: number; symbol?: string } & TagProps> = memo(
  ({ rewardPerSec, symbol, ...props }) => {
    ...
    const wayaPerDay = useMemo(() => {
      return rewardPerSec * 60 * 60 * 24
    }, [rewardPerSec])
    return (
      <Tag
        startIcon={<WayaOutlineIcon color={theme.colors.secondary} width={props.scale === 'sm' ? 13 : undefined} />}
        variant="secondary"
        {...props}
      >
        {wayaPerDay === 0 ? '0.00' : wayaPerDay.toFixed(2)} {symbol ?? t('WAYA')}{' '}
      </Tag>
    )
  },
)

const WayaOutlineIcon: React.FC<React.PropsWithChildren<SvgProps>> = memo((props) => {
  return (

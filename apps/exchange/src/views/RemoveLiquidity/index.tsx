...
import { Currency, Percent, WNATIVE } from '@plexswap/sdk-core'
import {
    AddIcon,
    ArrowDownIcon,
    AutoColumn,
...
import { useRemoveLiquidityFormState } from 'state/burn/reducer'
...
export default function RemoveLiquidity({ currencyA, currencyB, currencyIdA, currencyIdB }) {
  ...
  // burn state
  const { independentField, typedValue } = useRemoveLiquidityFormState()
  ...
  const handleSelectCurrencyA = useCallback(
    (currency: Currency) => {
      if (currencyIdB && currencyId(currency) === currencyIdB) {
        router.replace(`/core/remove/${currencyId(currency)}/${currencyIdA}`, undefined, { shallow: true })
      } else {
        router.replace(`/core/remove/${currencyId(currency)}/${currencyIdB}`, undefined, { shallow: true })
      }
    },
  ...
  const handleSelectCurrencyB = useCallback(
    (currency: Currency) => {
      if (currencyIdA && currencyId(currency) === currencyIdA) {
        router.replace(`/core/remove/${currencyIdB}/${currencyId(currency)}`, undefined, { shallow: true })
      } else {
        router.replace(`/core/remove/${currencyIdA}/${currencyId(currency)}`, undefined, { shallow: true })
      }
    },
    [currencyIdA, currencyIdB, router],
  )
            ...
            <LightGreyCard>
              ...
              {chainId && (oneCurrencyIsWNative || oneCurrencyIsNative) ? (
                <RowBetween style={{ justifyContent: 'flex-end', fontSize: '14px' }}>
                  {oneCurrencyIsNative ? (
                    <StyledInternalLink
                      href={`/core/remove/${currencyA?.isNative ? WNATIVE[chainId]?.address : currencyIdA}/${
...
export const RemoveLiquidityCoreLayout = ({ currencyA, currencyB, children }) => {
        ...
        <AppHeader
          backTo={
            isStable
              ...
              : `/core/pair/${currencyA?.address}/${currencyB?.address}`
          }

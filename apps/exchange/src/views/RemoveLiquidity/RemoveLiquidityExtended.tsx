...
import { CurrencyAmount, WNATIVE } from '@plexswap/sdk-core'
import {
    ArrowDownIcon,
    AutoColumn,
    AutoRow,
...
import { useDebouncedChangeHandler } from '@plexswap/hooks'
...
import { useDerivedExtendedBurnInfo } from 'hooks/extended/useDerivedExtendedBurnInfo'
...
import { useBurnExtendedActionHandlers } from './form/hooks'
...
export default function RemoveLiquidityExtended() {
...
function Remove({ tokenId }: { tokenId?: bigint }) {
  ...
  const { position } = useExtendedPositionFromTokenId(tokenId)
  ...
  const {
    ...
    outOfRange,
    error,
  } = useDerivedExtendedBurnInfo(position, percent, receiveWNATIVE)

  const { onPercentSelect } = useBurnExtendedActionHandlers()
  ...
  // const allowedSlippage = useUserSlippageToleranceWithDefault(DEFAULT_REMOVE_EXTENDED_LIQUIDITY_SLIPPAGE_TOLERANCE) // custom from users
      ...
      sendTransactionAsync({
        ...
        .then((response) => {
          ...
          addTransaction(
            { hash: response },
            {
              type: 'remove-liquidity-extended',
        ...
        <CardBody>
          ...
          {isStakedInCFExtended ? (
            <Message variant="primary" mb="20px">
              {t(
                'This liquidity position is currently staking in the Farm. Adding or removing liquidity will also harvest any unclaimed WAYA to your wallet.',
              )}
            </Message>
          ) : null}

...
import { useWayaTooltipContent } from './FarmBoosterCard'
import { useGetCalculatorMultiplier } from '../hooks/useGetBoostedAPR'
import LockDurationField from './WayaLockedDuration'

const WayaBlock = styled.div`
  ...
  border-radius: 16px;
`
interface WayaCalculatorProps {
  targetInputBalance: string
  earningTokenPrice: number
  lpTokenStakedAmount: BigNumber
  initialState?: any
  stakingTokenSymbol?: string
  setWayaMultiplier: (multiplier: number) => void
}

const WayaCalculator: React.FC<React.PropsWithChildren<WayaCalculatorProps>> = ({
  targetInputBalance,
  earningTokenPrice,
  initialState,
  stakingTokenSymbol = 'WAYA',
  lpTokenStakedAmount,
  setWayaMultiplier,
}) => {
  ...
  const { isLoading, lockedAmount, lockedStart, lockedEnd } = useUserLockedWayaStatus()
  ...
  const WayaMultiplier = useGetCalculatorMultiplier(userBalanceInFarm, lpTokenStakedAmount, userLockedAmount, duration)

  useEffect(() => {
    setWayaMultiplier(WayaMultiplier)
  }, [WayaMultiplier, setWayaMultiplier])
  ...
  } = useTooltip(t('Boost multiplier calculation does not include profit from WAYA staking pool'), {
    placement: 'top-end',
    tooltipOffset: [20, 10],
  })
...
export default WayaCalculator

export const getWayaMultiplier = (
  ...
  lMaxBoost: number,
  cDifficulties: number,
) => {
  const dB = userBalanceInFarm.times(lMaxBoost)
  ...
  const aB = aBPart1.dividedBy(cDifficulties).dividedBy(aBPart3)

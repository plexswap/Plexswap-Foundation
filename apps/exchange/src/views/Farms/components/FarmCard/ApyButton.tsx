...
import { Flex, RocketIcon, RoiCalculatorModal, Text, TooltipText, useModal, useTooltip } from '@plexswap/ui-plex'
...
const ApyButton: React.FC<React.PropsWithChildren<ApyButtonProps>> = ({
  ...
  const farm = useFarmFromPid(pid) as ExtendedFarmWithoutStakedValue | CoreFarmWithoutStakedValue | undefined
  ...
  const userBalanceInFarm = useMemo(() => {
    ...
    if (farm?.version  !== 11) {
      const wayaBalance = new BigNumber(farm?.wayaUserData?.stakedBalance ?? '0')
      if (wayaBalance.gt(0)) {
        return wayaBalance.plus(tokenBalance)
      }
    }
    return BIG_ZERO
  ...
  const [onPresentApyModal] = useModal(
    <RoiCalculatorModal
      ...
      earningTokenPrice={wayaPrice?.toNumber() ?? 0}

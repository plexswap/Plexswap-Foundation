...
import { getCrossFarmingSenderContract, getSpecialVaultContract } from 'utils/contractHelpers'
...
export const getSpecialVaultContractFee = async ({
  pid,
  amount,
  chainId,
  userAddress,
  ...
  try {
    const specialVaultContract = getSpecialVaultContract(undefined, chainId)
    ...
    const [encodeMessage, hasFirstTime, estimateGaslimit] = await Promise.all([
      specialVaultContract.read.encodeMessage([
    ...
    const calcFee = await specialVaultContract.read.calcFee([encodeMessage])

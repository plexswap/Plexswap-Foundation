...
const VaultStakeModal: React.FC<React.PropsWithChildren<VaultStakeModalProps>> = ({
  ...
  const usdValueStaked = new BigNumber(stakeAmount).times(wayaPrice)
  const formattedUsdValueStaked = wayaPrice.gt(0) && stakeAmount ? formatNumber(usdValueStaked.toNumber()) : ''
  const { flexibleApy } = useVaultApy()
  const { allowance, setLastUpdated } = useCheckVaultApprovalStatus(vaultKey)
  const { handleApprove: handleWayaApprove, pendingTx: wayaPendingTx } = useVaultApprove(vaultKey, setLastUpdated)
  ...
  const handleWithdrawal = async () => {
    // trigger withdrawAll function if the withdrawal will leave 0.00001 WAYA or less
    ...
    const receipt = await fetchWithCatchTxError(() => {
      ...
      if (pool.vaultKey === VaultKey.WayaFlexibleVault) {
        const { sharesAsBigNumber } = convertWayaToShares(convertedStakeAmount, pricePerFullShare || BIG_ZERO)
  ...
  const handleDeposit = async (lockDuration = 0) => {
    const receipt = await fetchWithCatchTxError(() => {
      // .toString() being called to fix a BigNumber error in prod
      // as suggested here https://github.com/ChainSafe/web3.js/issues/2077
      const extraArgs = pool.vaultKey === VaultKey.WayaVault ? [lockDuration.toString()] : []
  ...
  if (showRoiCalculator) {
    return (
      <VaultRoiCalculatorModal
        ...
        stakingTokenBalance={wayaAsBigNumber?.plus(stakingMax)}
  ...
  return (
    ...
    >
      ...
      <BalanceInput
        ...
        currencyValue={wayaPrice.gt(0) && `~${formattedUsdValueStaked || 0} USD`}
      ...
      {needEnable ? (
        <Button
          isLoading={wayaPendingTx}
          endIcon={wayaPendingTx ? <AutoRenewIcon spin color="currentColor" /> : null}
          onClick={handleWayaApprove}
          mt="24px"
        >
          {t('Enable')}
        </Button>

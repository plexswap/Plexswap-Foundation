...
import { MAX_LOCK_DURATION, VaultKey } from '@plexswap/pools'
import { AutoRenewIcon, Box, Button, Flex, Message, MessageText, Text } from '@plexswap/ui-plex'
import { getBalanceAmount, getDecimalAmount } from '@plexswap/utils/formatBalance'
...
const LockedModalBody: React.FC<React.PropsWithChildren<LockedModalBodyPropsType>> = ({
  ...
  const needsEnable = useMemo(() => wayaNeeded && !hasEnoughBalanceToExtend, [wayaNeeded, hasEnoughBalanceToExtend])

  const { allowance, setLastUpdated } = useCheckVaultApprovalStatus(VaultKey.WayaVault)
  const { handleApprove, pendingTx: approvePendingTx } = useVaultApprove(VaultKey.WayaVault, setLastUpdated)
  ...
  return (
    <>
      ...
      {!needsApprove && wayaNeeded ? (
        hasEnoughBalanceToExtend ? (
          <Text fontSize="12px" mt="24px">
            {t('0.0001 WAYA will be spent to extend')}
          </Text>
        ) : (
          <Message variant="warning" mt="24px">
            <MessageText maxWidth="200px">{t('0.0001 WAYA required for enabling extension')}</MessageText>
          </Message>
        )
      ) : null}
      ...
      <Flex mt="24px" flexDirection="column">
        {needsApprove ? (
          ...
          <ExtendEnable
            hasEnoughWaya={hasEnoughBalanceToExtend ?? false}

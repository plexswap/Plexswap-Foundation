...
import {
    ...
    SmartRouter,
    SmartRouterTrade,
} from '@plexswap/gateway-guardians/Ananke'
import { AetherRouter } from '@plexswap/gateway-guardians'
import {  BATCH_MULTICALL_CONFIGS } from '@plexswap/gateway-guardians/config'
import { useDebounce, usePropsChanged } from '@plexswap/hooks'
import { BigintIsh, Currency, CurrencyAmount, TradeType } from '@plexswap/sdk-core'
import { getPoolTypeKey, getRequestBody, parseAMMPriceResponse, parseQuoteResponse } from '@plexswap/trade-sentinels/Valorus'
...
>({
  ...
  return function useBestTrade({
    ...
    const candidatePoolsWithoutExtendedWithFot = useMemo(() => {
      let pools = candidatePools
      if (tokenInFee && tokenInFee.result.sellFeeBps > 0n) {
        pools = pools?.filter(
          (pool) =>
            !(
              pool.type === PoolType.EXTENDED &&
              baseCurrency &&
      ...
      if (tokenOutFee && tokenOutFee.result.buyFeeBps > 0n) {
        pools = pools?.filter(
          (pool) =>
            !(pool.type === PoolType.EXTENDED && currency && (pool.token0.equals(currency) || pool.token1.equals(currency))),
        )
      }
    ...
    const poolProvider = useMemo(
      () => SmartRouter.createStaticPoolProvider(candidatePoolsWithoutExtendedWithFot),
      [candidatePoolsWithoutExtendedWithFot],
    )
    ...
    const poolTypes = useMemo(() => {
      ...
      if (coreSwap) {
        types.push(PoolType.CORE)
      }
      if (extendedSwap) {
        types.push(PoolType.EXTENDED)
      }
      if (stableSwap) {
        types.push(PoolType.STABLE)
      }
      return types
    }, [coreSwap, extendedSwap, stableSwap])
...
type AetherGetBestTradeReturnType = Omit<Exclude<Awaited<ReturnType<typeof AetherRouter.getBestTrade>>, undefined>, 'graph'>

function createUseWorkerGetBestTradeOffchain() {
  return function useWorkerGetBestTradeOffchain(): (
    ...args: GetBestTradeParams
  ) => Promise<AetherGetBestTradeReturnType | null> {
    ...
    return useCallback(
      async (
        ...
        try {
          ...
          return AetherRouter.Transformer.parseTrade(currency.chainId, result) ?? null
        } catch (e) {
          console.error(e)
          throw new NoValidRouteError()
        }
...
export const useBestAMMTradeFromOffchainQuoter = bestTradeHookFactory<AetherRouter.AetherTradeWithoutGraph<TradeType>>({

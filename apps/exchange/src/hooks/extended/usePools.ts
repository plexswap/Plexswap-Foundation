import { BigintIsh, Currency, Token } from '@plexswap/sdk-core'
import { POOL_DEPLOYER_ADDRESSES, FeeAmount, Pool, computePoolAddress } from '@plexswap/sdk-extended'
import { extendedPoolStateABI } from 'config/abi/extendedPoolState'
...
export function usePools(
  ...
  const poolAddresses: (Address | undefined)[] = useMemo(() => {
    const extendedRootDeployerAddress = chainId && POOL_DEPLOYER_ADDRESSES[chainId]
    if (!extendedRootDeployerAddress) return new Array(poolTokens.length)

    return poolTokens.map((value) => value && PoolCache.getPoolAddress(extendedRootDeployerAddress, ...value))
  }, [chainId, poolTokens])

  const slot0s = useMultipleContractSingleData({
    addresses: poolAddresses,
    abi: extendedPoolStateABI,
    functionName: 'slot0',
  })

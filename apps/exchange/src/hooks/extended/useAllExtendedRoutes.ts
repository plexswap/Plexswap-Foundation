...
import { Pool, Route } from '@plexswap/sdk-extended'
import { useActiveChainId } from 'hooks/useActiveChainId'
import { useMemo } from 'react'
import { useExtendedSwapPools } from './useExtendedSwapPools'
 ...
 */
export function useAllExtendedRoutes(
  currencyIn?: Currency,
  currencyOut?: Currency,
): { loading: boolean; routes: Route<Currency, Currency>[] } {
  const { chainId } = useActiveChainId()
  const { pools, loading: poolsLoading } = useExtendedSwapPools(currencyIn, currencyOut)

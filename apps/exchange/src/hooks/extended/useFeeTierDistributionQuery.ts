...
export default function useFeeTierDistributionQuery(
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      return extendedClients[chainId].request(query, {
        token0: token0?.toLowerCase(),
        token1: token1?.toLowerCase(),
      })
    },

    enabled: Boolean(token0 && token1 && chainId && extendedClients[chainId]),
  ...
  return useQuery({
    ...
    queryFn: async ({ signal }) => {
      ...
      return explorerApiClient
        .GET('/cached/pools/extended/{chainName}/list/simple', {
          signal,
          params: {
            path: {

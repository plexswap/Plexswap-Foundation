...
import { FeeAmount, Pool, TICK_SPACINGS, tickToPrice } from '@plexswap/sdk-extended'
import { useMemo } from 'react'

import { PoolState, TickProcessed } from './types'
import useAllExtendedTicksQuery, { TickData } from './useAllExtendedTicksQuery'
...
function useTicksFromSubgraph(
  ...
  return useAllExtendedTicksQuery(poolAddress, 30000, enabled)
}

// Fetches all ticks for a given pool
export function useAllExtendedTicks(
  currencyA: Currency | undefined | null,
  currencyB: Currency | undefined | null,
  feeAmount: FeeAmount | undefined,
  enabled = true,
...
export function usePoolActiveLiquidity(
  ...
  const { isLoading, error, ticks } = useAllExtendedTicks(currencyA, currencyB, feeAmount)

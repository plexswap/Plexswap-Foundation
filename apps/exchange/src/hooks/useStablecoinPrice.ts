...
import { Currency, CurrencyAmount, Price, TradeType } from '@plexswap/sdk-core'
import { STABLE_COIN, WAYA } from '@plexswap/tokens'
...
import { computeTradePriceBreakdown } from 'views/Swap/ExtendedSwap/utils/exchange'
...
export function useStablecoinPrice(
  ...
  const isWaya = chainId && currency && WAYA[chainId] && currency.wrapped.equals(WAYA[chainId])

  const isStableCoin = currency && stableCoin && currency.wrapped.equals(stableCoin)

  const shouldEnabled = Boolean(
    currency && stableCoin && enabled && currentChainId === chainId && !isWaya && !isStableCoin,
  )
  ...
  const price = useMemo(() => {
    ...
    if (isWaya && wayaPrice) {
      return new Price(
        ...
        getFullDecimalMultiplier(stableCoin.decimals).times(wayaPrice.toFixed(stableCoin.decimals)).toString(),
      )
    }
    ...
    return undefined
  }, [currency, stableCoin, enabled, isWaya, wayaPrice, isStableCoin, priceFromApi, trade, hideIfPriceImpactTooHigh])

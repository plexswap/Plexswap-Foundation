...
import { SmartRouter } from '@plexswap/gateway-guardians/Ananke'
...
export type WorkerGetBestTradeOffchainEvent = [
  ...
  message: {
    ...
    params: AetherRouter.APISchema.RouterPostParams
  },
]
...
addEventListener('message', (event: MessageEvent<WorkerEvent>) => {
  ...
  if (message.cmd === 'getBestTradeOffchain') {
    const parsed = AetherRouter.APISchema.zRouterPostParams.safeParse(message.params)
    if (parsed.success === false) {
      postMessage([
        id,
        {
    ...
    AetherRouter.getBestTrade(currencyAAmount, currencyB, tradeType, {
      gasPriceWei: gasPrice,
      maxHops,
      maxSplits,
      ...
      .then((res) => {
        postMessage([
          ...
          {
            success: true,
            result: res && AetherRouter.Transformer.serializeTrade(res),
          },
        ])
      })
      .catch((err) => {

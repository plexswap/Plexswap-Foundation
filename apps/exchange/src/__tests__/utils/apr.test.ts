...
import { BIG_TEN, BIG_ZERO } from '@plexswap/utils/bigNumber'
...
describe('getFarmApr', () => {
  it(`returns null when parameters are missing`, () => {
    const { wayaRewardsApr, lpRewardsApr } = getFarmApr(ChainId.BSC, null, null, null, null, 40)
    expect(wayaRewardsApr).toBeNull()
    expect(lpRewardsApr).toEqual(0)
  })
  it(`returns null when APR is infinite`, () => {
    const { wayaRewardsApr, lpRewardsApr } = getFarmApr(ChainId.BSC, BIG_ZERO, BIG_ZERO, BIG_ZERO, '', 40)
    expect(wayaRewardsApr).toBeNull()
    expect(lpRewardsApr).toEqual(0)
  })
  it(`get the correct pool APR`, () => {
    ...
    expect(wayaRewardsApr).toEqual(4204800)
    expect(lpRewardsApr).toEqual(0)
  })

...
import { SerializedPoolAddon, SUPPORTED_CHAIN_IDS, getPoolsConfig } from '@plexswap/pools'
...
describe('transformPool', () => {
  ...
  for (const poolsConfig of poolsConfigs) {
    ...
    const poolTable: [
      number,
      Pool.SerializedPoolConfig<SerializedWrappedToken>,
    ][] = poolsConfig?.map((poolsConfigItem) => [poolsConfigItem.poolId, poolsConfigItem]) || []
    it.each(poolTable)('transforms pool %d correctly', (poolId, config) => {
      const pool = {
        ...config,
        totalStaked: '10',
        stakingLimit: '10',
      ...
      } as SerializedPoolAddon
      const transformedPool = transformPool(pool)

      expect(transformedPool).toHaveProperty('poolId', poolId)

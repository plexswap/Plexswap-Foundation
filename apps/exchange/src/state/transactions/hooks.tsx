...
export function usePendingTransactions(): {
  ...
  specialFarmPendingList: SpecialPendingData[]
} {
  const allTransactions = useAllTransactions()
  ...
  const pending = sortedRecentTransactions
    .filter((tx) => !tx.receipt || tx?.specialFarm?.status === FarmTransactionStatus.PENDING)
    .map((tx) => tx.hash)
  const hasPendingTransactions = !!pending.length

  const specialFarmPendingList = sortedRecentTransactions
    .filter((tx) => pending.includes(tx.hash) && !!tx.specialFarm)
    .map((tx) => ({ txid: tx?.hash, lpAddress: tx?.specialFarm?.lpAddress, type: tx?.specialFarm?.type }))

  return {
    hasPendingTransactions,
    specialFarmPendingList,
    pendingNumber: pending.length,
  }
}

export function useSpecialFarmPendingTransaction(lpAddress?: string): SpecialPendingData[] {
  const { specialFarmPendingList } = usePendingTransactions()
  return useMemo(() => {
    if (!lpAddress) return []
    return specialFarmPendingList.filter((tx) => tx?.lpAddress?.toLowerCase() === lpAddress.toLowerCase())
  }, [lpAddress, specialFarmPendingList])
}

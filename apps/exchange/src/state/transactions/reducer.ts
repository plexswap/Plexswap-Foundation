...
export default createReducer(initialState, (builder) =>
  builder
    .addCase(
      ...
      (
        transactions,
        { payload: { chainId, from, hash, approval, summary, translatableSummary, claim, type, order, specialFarm } },
      ) => {
    ...
    .addCase(finalizeTransaction, (transactions, { payload: { hash, chainId, receipt, specialFarm } }) => {
      const tx = transactions[chainId]?.[hash]
      if (!tx) {
        return
      }
      ...
      if (tx.type === 'limit-order-submission') {
        ...
        if (tx.specialFarm?.steps[0].status === FarmTransactionStatus.PENDING) {
          if (receipt.status === FarmTransactionStatus.FAIL) {
            tx.specialFarm = { ...tx.specialFarm, status: receipt.status }
          }

          tx.specialFarm.steps[0] = {
            ...tx.specialFarm.steps[0],
            status: receipt.status ?? FarmTransactionStatus.PENDING,
          }
        } else {
          tx.specialFarm = specialFarm
        }
      }
    })

...
interface OverviewResponse {
  plexFactories: PlexFactory[]
  factories?: PlexFactory[]
}
/**
...
const getOverviewData = async (
  ...
  const factoryString = checkIsStableSwap() ? `factories` : `plexFactories`
  try {
...
const formatPlexFactoryResponse = (rawPlexFactory?: PlexFactory[]) => {
  if (rawPlexFactory) {
    return rawPlexFactory.reduce(
      (acc, cur) => {
...
const useFetchProtocolData = (): ProtocolFetchState => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const overviewData = formatPlexFactoryResponse(data?.plexFactories)
      const overviewData24 = formatPlexFactoryResponse(data24?.plexFactories)
      const overviewData48 = formatPlexFactoryResponse(data48?.plexFactories)
...
export const fetchProtocolData = async (chainName: MultiChainName, block24: Block, block48: Block) => {
  ...
  if (data?.factories && data.factories.length > 0) data.plexFactories = data.factories
  if (data24?.factories && data24.factories.length > 0) data24.plexFactories = data24.factories
  if (data48?.factories && data48.factories.length > 0) data48.plexFactories = data48.factories

...
import { Currency, CurrencyAmount, Percent, Price, Token, ZERO } from "@plexswap/sdk-core";
import { BIG_ZERO } from "@plexswap/utils/bigNumber";
import { formatFraction, formatPercent, formatPrice } from "@plexswap/utils/formatFractions";
import { isPositionOutOfRange } from "@plexswap/utils/isPositionOutOfRange";
import { FeeAmount, FeeCalculator, TickMath, sqrtRatioX96ToPrice } from "@plexswap/sdk-extended";
import BigNumber from "bignumber.js";
import { useCallback, useMemo, useState } from "react";

import { Button, DynamicSection, Flex, useMatchBreakpoints } from "@plexswap/ui-plex";

import { ScrollableContainer } from "@plexswap/ui-plex/components/RoiCalculatorModal/RoiCalculatorModal";
...
export function RoiCalculator({
  ...
  const [editWayaPrice, setEditWayaPrice] = useState<number | null>(null);
  ...
  const wayaAprFactor = farmingRewardsEnabled && props.wayaAprFactor;

  const wayaPriceDiffPercent =
    farmingRewardsEnabled && props.wayaPrice && editWayaPrice && editWayaPrice / +props.wayaPrice;

  const derivedWayaApr = useMemo(() => {
    if (
      !amountA ||
      !amountB ||
    ...
    try {
      ...
      const wayaApr =
        positionLiquidity > ZERO
          ? new BigNumber(positionLiquidity.toString()).times(wayaAprFactor).div(usdValue)
          : BIG_ZERO;
  ...
  }, [amountA, amountB, priceRange, sqrtRatioX96, farmingRewardsEnabled, wayaAprFactor, tickCurrent, usdValue]);

  const editedWayaApr = useMemo(
    () =>
      derivedWayaApr && typeof wayaPriceDiffPercent === "number"
        ? derivedWayaApr.times(wayaPriceDiffPercent)
        : derivedWayaApr,
    [wayaPriceDiffPercent, derivedWayaApr]
  );
  ...
  const {
    ...
    compoundOn,
    wayaApr: farmingRewardsEnabled && derivedWayaApr ? derivedWayaApr.toNumber() : undefined,
    editWayaApr: farmingRewardsEnabled && editedWayaApr ? editedWayaApr.toNumber() : undefined,
  });
    ...
    <>
      <ScrollableContainer>
        ...
        <ImpermanentLossCalculator
          ...
          wayaReward={originalWayaReward}
          wayaPrice={farmingRewardsEnabled ? props.wayaPrice : undefined}
          setEditWayaPrice={setEditWayaPrice}
        />
        <AnimatedArrow state={{}} />
      ...
      <Details
        ...
        farmApr={farmingRewardsEnabled ? editWayaApr || wayaApr : undefined}
        farmApy={farmingRewardsEnabled ? editWayaApy || wayaApy : undefined}

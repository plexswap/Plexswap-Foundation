...
import { Currency, CurrencyAmount, Pair, Price, Trade, TradeType } from '@plexswap/sdk-core'

import { getBestTradeFromCoreExactIn } from './getBestTradeFromCore'
import { getStableSwapFee, getStableSwapOutputAmount } from './onchain'
import { createTradeWithStableSwap, createTradeWithStableSwapFromCoreTrade, getFeePercent } from './stableSwap'
...
export async function getBestTradeWithStableSwap(
  ...
  if (!stableSwapPairs.length) {
    return createTradeWithStableSwapFromCoreTrade(baseTrade)
  }
  ...
  const getLatestOutputAmount = async () => {
    ...
    return shouldRecalculateOutputAmount() ? getOutputAmountFromCore(outputAmount, outputToken, options) : outputAmount
  }
  ...
  for (const [index, pair] of route.pairs.entries()) {
    ...
    if (stableSwapPair) {
      ...
      setCurrentRouteType(RouteType.STABLE)
      continue
    }
    ...
    setCurrentRouteType(RouteType.CORE)
  }
...
async function getOutputAmountFromCore<TInput extends Currency, TOutput extends Currency>(
  ...
  const trade = await getBestTradeFromCoreExactIn(inputAmount, outputToken, options)

...
import { Currency, CurrencyAmount, Percent, TradeType, validateAndParseAddress } from '@plexswap/sdk-core'
...
import { PlexswapOptions } from '../types'

// Wrapper for plexswap router-sdk trade entity to encode swaps for Evolus Router
export class PlexswapTrade implements Command {
  readonly tradeType: RouterTradeType = RouterTradeType.PlexswapTrade

  readonly type: TradeType

  constructor(public trade: SmartRouterTrade<TradeType>, public options: PlexswapOptions) {
  ...
  encode(planner: RoutePlanner): void {
    ...
    for (const route of trade.routes) {
      ...
      if (route.type === RouteType.CORE) {
        addCoreSwap(planner, singleRouteTrade, this.options, routerMustCustody, payerIsUser)
        continue
      }
      if (route.type === RouteType.EXTENDED) {
        addExtendedSwap(planner, singleRouteTrade, this.options, routerMustCustody, payerIsUser)
        continue
      }
...
function addCoreSwap(
  ...
  invariant(trade.routes.length === 1 && trade.routes[0].type === RouteType.CORE, 'Only allow single route v2 trade')
  ...
  // same as encodeCoreSwap only we dont return calldatas instead we push to the command planner
  if (trade.tradeType === TradeType.EXACT_INPUT) {
    planner.addCommand(CommandType.CORE_SWAP_EXACT_IN, [
  ...
  planner.addCommand(CommandType.CORE_SWAP_EXACT_OUT, [recipient, amountOut, amountIn, path, payerIsUser])
}

// encode a extended swap
function addExtendedSwap(
  planner: RoutePlanner,
  trade: SmartRouterTrade<TradeType>,
  options: PlexswapOptions,
  routerMustCustody: boolean,
  ...
  invariant(trade.routes.length === 1 && trade.routes[0].type === RouteType.EXTENDED, 'Only allow single route extended trade')
  const [route] = trade.routes

  const { inputAmount, outputAmount } = route

  // we need to generaate extended path as a hash string. we can still use encodeMixedRoute
  // as a extended swap is essentially a for of mixedRoute
  ...
  if (trade.tradeType === TradeType.EXACT_INPUT) {
    const exactInputSingleParams: ABIParametersType<CommandType.EXTENDED_SWAP_EXACT_IN> = [
      recipient,
      amountIn,
      amountOut,
      path,
    ...
    planner.addCommand(CommandType.EXTENDED_SWAP_EXACT_IN, exactInputSingleParams)
    return
  }

  const exactOutputSingleParams: ABIParametersType<CommandType.EXTENDED_SWAP_EXACT_OUT> = [
    recipient,
    amountOut,
    amountIn,
    path,
  ...
  planner.addCommand(CommandType.EXTENDED_SWAP_EXACT_OUT, exactOutputSingleParams)
}
...
// encode a mixed route swap, i.e. including both v2 and extended pools
function addMixedSwap(
  ...
  const mixedRouteIsAllExtended = (r: Omit<BaseRoute, 'input' | 'output'>) => {
    return r.pools.every(SmartRouter.isExtendedPool)
  }
  const mixedRouteIsAllV2 = (r: Omit<BaseRoute, 'input' | 'output'>) => {
    return r.pools.every(SmartRouter.isCorePool)
  }
  const mixedRouteIsAllStable = (r: Omit<BaseRoute, 'input' | 'output'>) => {
    return r.pools.every(SmartRouter.isStablePool)
  }

  // Check if a mixed route is actually pure v2, extended or stable route.
  // If that's the case then let the corresponding command adder to handle the encode
  if (mixedRouteIsAllExtended(route)) {
    addExtendedSwap(
      planner,
      {
        ...trade,
  ...
  for (let i = 0; i < sections.length; i++) {
    ...
    const nextIsV2 = nextSection.length && nextSection.every(SmartRouter.isCorePool)
    ...
    switch (newRoute.type) {
      case RouteType.EXTENDED: {
        const path = SmartRouter.encodeMixedRouteToPath(newRoute, false)
        planner.addCommand(CommandType.EXTENDED_SWAP_EXACT_IN, [currentRecipient, inAmount, outAmount, path, payByUser])
        break
      }
      case RouteType.CORE: {
        const path = newRoute.path.map((token) => token.wrapped.address)
        planner.addCommand(CommandType.CORE_SWAP_EXACT_IN, [currentRecipient, inAmount, outAmount, path, payByUser])
        break
      }

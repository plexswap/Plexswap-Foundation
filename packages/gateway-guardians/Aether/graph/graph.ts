import { Currency, CurrencyAmount, Fraction, TradeType } from '@plexswap/sdk-core'
import { formatFraction } from '@plexswap/utils/formatFractions'
import memoize from 'lodash/memoize.js'
import invariant from 'tiny-invariant'
import { Address } from 'viem'

import { createPoolQuoteGetter } from '../../Ananke/providers'
import { Pool } from '../../Ananke/types'
import { buildBaseRoute, getPoolAddress, isStablePool } from '../../Ananke/utils'
import { estimateGasCost } from '../gasCost'
import { getCurrencyPairs } from '../pool'
import { Edge, Graph, TradeConfig, AetherRoute, AetherTrade, Vertice } from '../types'
...
export async function findBestTrade(params: FindBestTradeParams): Promise<AetherTrade<TradeType> | undefined> {
...
async function getBestTrade({
  ...
  tradeType,
  maxHops = 3,
}: FindBestTradeParams): Promise<AetherTrade<TradeType> | undefined> {
  const isExactIn = tradeType === TradeType.EXACT_INPUT
  ...
  const getQuoteAmount = (r: AetherRoute) => (isExactIn ? r.outputAmount : r.inputAmount)

  const buildTrade = (g: Graph, routes: AetherRoute[]): AetherTrade<TradeType> => {
    const {
      gasUseEstimate,
      quoteAmount,
      gasUseEstimateQuote,
  ...
  async function findBestRoute(amount: CurrencyAmount<Currency>): Promise<Omit<AetherRoute, 'percent'> | undefined> {

...
import { Currency, CurrencyAmount, ERC20Token, Native, Percent, TradeType } from '@plexswap/sdk-core'
import { ADDRESS_ZERO, Tick } from '@plexswap/sdk-extended'
import { Address } from 'viem'
import { ExtendedPool, Pool, PoolType, Route, SmartRouterTrade, StablePool, CorePool } from '../types'
...
export interface SerializedCorePool extends Omit<CorePool, 'reserve0' | 'reserve1'> {
  reserve0: SerializedCurrencyAmount
  reserve1: SerializedCurrencyAmount
}

export interface SerializedExtendedPool
  extends Omit<
    ExtendedPool,
    | 'token0'
    | 'token1'
...
export type SerializedPool = SerializedCorePool | SerializedExtendedPool | SerializedStablePool
...
export function serializePool(pool: Pool): SerializedPool {
  ...
  throw new Error('Cannot serialize unsuported pool')
}
...
export function parsePool(chainId: ChainId, pool: SerializedPool): Pool {
  ...
  if (pool.type === PoolType.EXTENDED) {
    return {
      ...pool,

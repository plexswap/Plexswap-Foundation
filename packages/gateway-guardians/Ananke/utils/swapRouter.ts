...
import { Currency, CurrencyAmount, Percent, TradeType, WNATIVE, validateAndParseAddress } from '@plexswap/sdk-core'
import { FeeOptions, MethodParameters, Payments, PermitOptions, Position, SelfPermit, toHex } from '@plexswap/sdk-extended'
import invariant from 'tiny-invariant'
import { Address, Hex, encodeFunctionData } from 'viem'
import { plexRouterABI } from '../../config/abis/IPlexswapRouter'
import { ADDRESS_THIS, MSG_SENDER } from '../constants'
import { BaseRoute, ExtendedPool, RouteType, SmartRouterTrade, StablePool } from '../types'
...
import { isExtendedPool, isStablePool, isCorePool } from './pool'
import { buildBaseRoute } from './route'

const ZERO = 0n
const REFUND_PRICE_IMPACT_THRESHOLD = new Percent(50n, 100n)
...
/**
 * Represents the Plexswap Core + Extended + StableSwap, and has static methods for helping execute trades.
 */
export abstract class SwapRouter {
  public static ABI = plexRouterABI
  ...
  /**
   * @notice Generates the calldata for a Swap with a Core Route.
   * @param trade The CoreTrade to encode.
   * @param options SwapOptions to use for the trade.
   * @param routerMustCustody Flag for whether funds should be sent to the router
   * @param performAggregatedSlippageCheck Flag for whether we want to perform an aggregated slippage check
   * @returns A string array of calldatas for the trade.
   */
  private static encodeCoreSwap(
    ...
    // Core trade should have only one route
  ...
  /**
   * @notice Generates the calldata for a Swap with a Extended Route.
   * @param trade The ExtendedTrade to encode.
   * @param options SwapOptions to use for the trade.
   * @param routerMustCustody Flag for whether funds should be sent to the router
   * @param performAggregatedSlippageCheck Flag for whether we want to perform an aggregated slippage check
   * @returns A string array of calldatas for the trade.
   */
  private static encodeExtendedSwap(
    trade: SmartRouterTrade<TradeType>,
    options: SwapOptions,
    routerMustCustody: boolean,
    performAggregatedSlippageCheck: boolean,
      ...
      if (singleHop) {
        if (trade.tradeType === TradeType.EXACT_INPUT) {
          const exactInputSingleParams = {
            ...
            fee: (pools[0] as ExtendedPool).fee,
            recipient,
            amountIn,
   ...
   *         them via the existing encodeExtendedSwap and encodeCoreSwap methods.
  ...
  private static encodeMixedRouteSwap(
    ...
    for (const route of trade.routes) {
      ...
      const mixedRouteIsAllExtended = (r: Omit<BaseRoute, 'input' | 'output'>) => {
        return r.pools.every(isExtendedPool)
      }
      const mixedRouteIsAllCore = (r: Omit<BaseRoute, 'input' | 'output'>) => {
        return r.pools.every(isCorePool)
      }
      ...
      if (singleHop) {
        /// For single hop, since it isn't really a mixedRoute, we'll just mimic behavior of Extended or Core
        /// We don't use encodeExtendedSwap() or encodeCoreSwap() because casting the trade to a ExtendedTrade or CoreTrade is overcomplex
        if (mixedRouteIsAllExtended(route)) {
          calldatas = [
            ...calldatas,
            ...SwapRouter.encodeExtendedSwap(
              {
                ...trade,
                routes: [route],
                inputAmount,
        ...
        } else if (mixedRouteIsAllCore(route)) {
          calldatas = [
            ...calldatas,
            SwapRouter.encodeCoreSwap(
              {
                ...trade,
                routes: [route],
                inputAmount,
        ...
        for (let i = 0; i < sections.length; i++) {
          ...
          if (mixedRouteIsAllExtended(newRoute)) {
          ...
          } else if (mixedRouteIsAllCore(newRoute)) {
  ...
  private static encodeSwaps(
    ...
    for (const trade of trades) {
      if (trade.routes.length === 1 && trade.routes[0].type === RouteType.CORE) {
        calldatas.push(SwapRouter.encodeCoreSwap(trade, options, routerMustCustody, performAggregatedSlippageCheck))
      } else if (trade.routes.every((r) => r.type === RouteType.EXTENDED)) {
        for (const calldata of SwapRouter.encodeExtendedSwap(
          trade,
          options,
          routerMustCustody,
          performAggregatedSlippageCheck,
  ...
  public static riskOfPartialFill(trades: AnyTradeType): boolean {
    if (Array.isArray(trades)) {
      return trades.some((trade) => {
        return SwapRouter.extendedTradeWithHighPriceImpact(trade)
      })
    }
    return SwapRouter.extendedTradeWithHighPriceImpact(trades)
  }

  private static extendedTradeWithHighPriceImpact(trade: SmartRouterTrade<TradeType>): boolean {
    return (
      !(trade.routes.length === 1 && trade.routes[0].type === RouteType.CORE) &&
      getPriceImpact(trade).greaterThan(REFUND_PRICE_IMPACT_THRESHOLD)
    )
  }

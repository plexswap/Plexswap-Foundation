...
import { ANANKE_FEE_PATH_PLACEHOLDER } from './../constants'
import { BaseRoute, Pool } from '../types'
import { getOutputCurrency, isExtendedPool } from './pool'
...
export function encodeMixedRouteToPath(route: BaseRoute, exactOutput: boolean): Hex {
  ...
  const { path, types } = route.pools.reduce(
    (
      ...
      const fee = isExtendedPool(pool) ? pool.fee : ANANKE_FEE_PATH_PLACEHOLDER
      if (index === 0) {
        return {

...
import { BigintIsh, Currency, CurrencyAmount } from '@plexswap/sdk-core'
import { AbortControl, isAbortError } from '@plexswap/utils/abortControl'
import retry from 'async-retry'
import { Abi, Address } from 'viem'

import { mixedRouteQuoterABI } from '../../config/abis/IMixedRouteQuoter'
import { quoterCoreABI } from '../../config/abis/IQuoterCore'
import { EXTENDED_QUOTER_ADDRESSES, MIXED_ROUTE_QUOTER_ADDRESSES } from '../../config/constants'
import { BATCH_MULTICALL_CONFIGS } from '../../config/constants/multicall'
...
import { encodeMixedRouteToPath, getQuoteCurrency, isExtendedPool, isStablePool, isCorePool } from '../utils'
import { Result } from './multicallProvider'
import { PlexMulticallProvider } from './multicallSwapProvider'
...
const SUCCESS_RATE_CONFIG = {
  ...
  [ChainId.BSC]: 0.1,
  [ChainId.PLEXCHAIN]: 0.1,
} as const satisfies Record<ChainId, number>

type ExtendedInputs = [string, string]
type MixedInputs = [string, number[], string]
type CallInputs = ExtendedInputs | MixedInputs
    ...
    const createGetRoutesWithQuotes = (isExactIn = true) => {
      ...
      return async function getRoutesWithQuote(
        ...
        const multicallConfigs =
          ...
          BATCH_MULTICALL_CONFIGS[ChainId.PLEXCHAIN]
        const {
        ...
        const multicall2Provider = new PlexMulticallProvider(chainId, chainProvider, defaultGasLimitPerCall)
...
export const createExtendedOnChainQuoteProvider = onChainQuoteProviderFactory({
  getQuoterAddress: (chainId) => EXTENDED_QUOTER_ADDRESSES[chainId],

...
import { FeeAmount, Pool, parseProtocolFees } from '@plexswap/sdk-extended'
import type { GraphQLClient } from 'graphql-request'
import { gql } from 'graphql-request'
import memoize from 'lodash/memoize.js'
import { Address, getAddress } from 'viem'

import { ExtendedPoolWithTvl, PoolType, SubgraphProvider, CorePoolWithTvl, WithTvl } from '../../types'
import { computeCorePoolAddress, logger, metric } from '../../utils'
...
const getExtendedPoolMeta = memoize(
...
const getExtendedPoolMetas = memoize(
  (pair: [Currency, Currency]) =>
    [FeeAmount.LOWEST, FeeAmount.LOW, FeeAmount.MEDIUM, FeeAmount.HIGH].map((fee) => getExtendedPoolMeta([...pair, fee])),
  ([currencyA, currencyB]) => {
    if (currencyA.wrapped.equals(currencyB.wrapped)) {
...
interface ExtendedPoolSubgraphResult {
  id: string
  liquidity: string
  sqrtPrice: string
  tick: string
...
export const getExtendedPoolSubgraph = subgraphPoolProviderFactory<ExtendedPoolMeta, ExtendedPoolWithTvl>({
  id: 'Extended',
  getPoolMetas: getExtendedPoolMetas,
  getPoolsFromSubgraph: async ({ addresses, getPoolMetaByAddress, client }) => {
    const { pools: poolsFromSubgraph } = await client.request<{ pools: ExtendedPoolSubgraphResult[] }>(queryExtendedPools, {
      pageSize: 1000,
      poolAddrs: addresses,
    })
    ...
    return poolsFromSubgraph.map(({ id, liquidity, sqrtPrice, tick, totalValueLockedUSD, feeProtocol }) => {
      ...
      return {
        type: PoolType.EXTENDED as const,
        fee,
        token0,
        token1,
...
interface CorePoolSubgraphResult {
  id: string
  reserveUSD: string
  reserve0: string
  reserve1: string
...
export const getCorePoolSubgraph = subgraphPoolProviderFactory<PoolMeta, CorePoolWithTvl>({
  id: 'Core',
  getPoolMetas: ([currencyA, currencyB]) => [
    {
      currencyA,
      currencyB,
      address: computeCorePoolAddress(currencyA.wrapped, currencyB.wrapped),
    },
  ],
  getPoolsFromSubgraph: async ({ addresses, getPoolMetaByAddress, client }) => {
    const { pairs: poolsFromSubgraph } = await client.request<{ pairs: CorePoolSubgraphResult[] }>(queryCorePools, {
      pageSize: 1000,
      poolAddrs: addresses,
    })
...
const queryAllExtendedPools = gql`
...
export interface ExtendedDetailedPoolSubgraphResult extends ExtendedPoolSubgraphResult {
  token0: TokenFromSubgraph
  token1: TokenFromSubgraph
}

export const getAllExtendedPoolsFromSubgraph = subgraphAllPoolsQueryFactory<ExtendedPoolWithTvl>({
  id: 'getAllExtendedPoolsFromSubgraph',
  getPoolsFromSubgraph: async ({ lastId, pageSize, client, chainId }) => {
    const { pools: poolsFromSubgraph } = await client.request<{ pools: ExtendedDetailedPoolSubgraphResult[] }>(
      queryAllExtendedPools,
      {
        pageSize,
        id: lastId,
      },

...
import { getPoolAddress, isExtendedPool, isCorePool, logger } from '../utils'
...
export function getBestSwapRouteBy(
              ...
              .map(
                (r) =>
                  `${r.percent}% ${r.amount.toExact()} ${r.pools
                    .map((p) => {
                      if (isCorePool(p)) {
                        return `Core ${p.reserve0.currency.symbol}-${p.reserve1.currency.symbol}`
                      }
                      if (isExtendedPool(p)) {
                        return `Extended fee ${p.fee} ${p.token0.symbol}-${p.token1.symbol}`
                      }
    ...
    while (layer > 0) {
      ...
      for (let i = percentIndex; i >= 0; i--) {
        ...
        if (remainingPercentNew === 0 && splits >= minSplits) {
          ...
          //   const onlyExtendedRoutes = curRoutesNew.every((route) => route.protocol == Protocol.Extended)

          //   if (gasModel == undefined || !onlyExtendedRoutes) {
          //     throw new Error("Can't compute L1 gas fees.")
          //   } else {
          //     const gasCostL1 = await gasModel.calculateL1GasFees!(curRoutesNew as ExtendedRouteWithValidQuote[])
  ...
  // if (HAS_L1_FEE.includes(chainId)) {
  //   // ensure the gasModel exists and that the swap route is a extended only route
  //   const onlyExtendedRoutes = bestSwap.every((route) => route.protocol == Protocol.Extended)
  //   if (gasModel == undefined || !onlyExtendedRoutes) {
  //     throw new Error("Can't compute L1 gas fees.")
  //   } else {
  //     gasCostsL1ToL2 = await gasModel.calculateL1GasFees!(bestSwap as ExtendedRouteWithValidQuote[])
  //   }
  // }

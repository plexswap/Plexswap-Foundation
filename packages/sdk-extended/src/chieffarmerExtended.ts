import { BigintIsh, CurrencyAmount, ONE, Token, validateAndParseAddress, ZERO } from '@plexswap/sdk-core'
import invariant from 'tiny-invariant'
import { encodeFunctionData, Hex } from 'viem'
import { chiefFarmerExtendedABI } from './abi/ChiefFarmerExtended'
...
export abstract class ChiefFarmerExtended {
  public static ABI = chiefFarmerExtendedABI
  ...
  public static collectCallParameters(options: CollectOptions): MethodParameters {
    const calldatas: Hex[] = ChiefFarmerExtended.encodeCollect(options)
  ...
  public static removeCallParameters(position: Position, options: RemoveLiquidityOptions): MethodParameters {
    ...
    if (options.permit) {
      ...
      // calldatas.push(
      //   ChiefFarmerExtended.INTERFACE.encodeFunctionData('permit', [
    ...
    calldatas.push(
      ...ChiefFarmerExtended.encodeCollect({
    ...
    if (options.liquidityPercentage.equalTo(ONE)) {
      if (options.burnToken) {
        calldatas.push(encodeFunctionData({ abi: ChiefFarmerExtended.ABI, functionName: 'burn', args: [tokenId] }))
      }
    } else {

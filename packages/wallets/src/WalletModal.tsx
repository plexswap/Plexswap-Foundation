import { usePreloadImages } from '@plexswap/hooks'
...
export type WalletConfigCore<T = unknown> = {
  id: string
  title: string
...
interface WalletModalCoreProps<T = unknown> extends ModalCoreProps {
  wallets: WalletConfigCore<T>[]
...
const selectedWalletAtom = atom<WalletConfigCore<unknown> | null>(null)

export function useSelectedWallet<T>() {
  // @ts-ignore
  return useAtom<WalletConfigCore<T> | null>(selectedWalletAtom)
}
...
function MobileModal<T>({
  ...
  docLink,
  docText,
}: Pick<WalletModalCoreProps<T>, 'wallets' | 'docLink' | 'docText'> & {
  connectWallet: (wallet: WalletConfigCore<T>) => void
}) {
  const { t } = useTranslation()
  ...
  const installedWallets: WalletConfigCore<T>[] = wallets.filter((w) => w.installed)
  const walletsToShow: WalletConfigCore<T>[] = wallets.filter((w) => {
...
function WalletSelect<T>({
  ...
  wallets: WalletConfigCore<T>[]
  onClick: (wallet: WalletConfigCore<T>) => void
  displayCount?: number
}) {
  const { t } = useTranslation()
...
function sortWallets<T>(wallets: WalletConfigCore<T>[], lastUsedWalletName: string | null) {
...
function DesktopModal<T>({
  ...
  const wallets: WalletConfigCore<T>[] = wallets_.filter((w) => {
  ...
  const connectToWallet = (wallet: WalletConfigCore<T>) => {
    connectWallet(wallet)
  }
...
export function WalletModalCore<T = unknown>(props: WalletModalCoreProps<T>) {
  ...
  const imageSources = useMemo(
    () =>
      wallets
        ...
        .concat('https://cdn.plexswap.com/wallets/wallet_intro.png') as string[],
    [wallets],
  )

  usePreloadImages(imageSources.slice(0, MOBILE_DEFAULT_DISPLAY_COUNT))

  const connectWallet = (wallet: WalletConfigCore<T>) => {
    setSelected(wallet)
    setError('')
  ...
  return (
    <ModalCore closeOnOverlayClick disableOutsidePointerEvents={false} {...rest}>
...
const Intro = ({ docLink, docText }: { docLink: string; docText: string }) => {
  ...
  return (
    <>
      ...
      <Image src="https://assets.plexfinance.us/images/wallets/wallet_intro.png" width={198} height={178} />
...
const NotInstalled = ({ wallet, qrCode }: { wallet: WalletConfigCore; qrCode?: string }) => {
  const { t } = useTranslation()
  return (
    <>
      <Heading as="h1" fontSize="20px" color="secondary">

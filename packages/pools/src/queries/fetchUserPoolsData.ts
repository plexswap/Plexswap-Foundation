...
import { SerializedPool, OnChainProvider } from '../types'
...
const getNonMasterPools = getPoolsFactory((pool) => pool.poolId !== 0)
...
export const fetchPoolsAllowance = async ({ account, chainId, provider }: FetchUserDataParams) => {
  ...
  return fromPairs(
    nonBnbPools.map((pool, index) => [pool.poolId, new BigNumber(allowances[index].toString()).toJSON()]),
  )
}
  ...
  const poolTokenBalances = fromPairs(
    nonBnbPools
      .map<[number, string] | null>((pool) => {
        ...
        return [pool.poolId, new BigNumber(tokenBalances[pool.stakingToken.address].toString()).toJSON()]
      })
...
export const fetchUserStakeBalances = async ({ account, chainId, provider }: FetchUserDataParams) => {
  ...
  const userInfo = await client.multicall({
    contracts: nonMasterPools.map(
      ({ contractAddress }) =>
        ({
          abi: cropChiefABI.filter((r) => r.name === 'userInfo'),
  ...
  return fromPairs(
    nonMasterPools.map((pool, index) => [pool.poolId, new BigNumber(userInfo[index][0].toString()).toJSON()]),
  )
}
...
export const fetchUserPendingRewards = async ({ account, chainId, provider }: FetchUserDataParams) => {
  ...
  const res = await client.multicall({
    contracts: nonMasterPools.map(
      ({ contractAddress }) =>
        ({
          abi: cropChiefABI.filter((r) => r.name === 'pendingReward'),
          address: contractAddress,
  ...
  return fromPairs(nonMasterPools.map((pool, index) => [pool.poolId, new BigNumber(res[index].toString()).toJSON()]))
}

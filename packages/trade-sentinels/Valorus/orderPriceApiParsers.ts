...
import { ExclusiveDutchOrder, createExclusiveDutchOrderTrade } from './../Hermes'
import { Currency, CurrencyAmount, Percent, TradeType, type BigintIsh } from '@plexswap/sdk-core'
import { PoolType, AetherRouter } from '@plexswap/gateway-guardians'
...
>(
  ...
  if (bestOrder.type === OrderType.DUTCH_LIMIT) {
    ...
    return {
      ...
      ammTrade: otherAmmTrade ? AetherRouter.Transformer.parseTrade(chainId, otherAmmTrade.order as AMMOrder) : undefined,
...
): AetherRouter.AetherTradeWithoutGraph<TradeType> & { gasUseEstimateUSD: number } {
  const {

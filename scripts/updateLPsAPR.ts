...
import { stableSwapClient, coreClients } from '../apps/exchange/src/utils/graphql'
...
const getAprsForFarmGroup = async (addresses: string[], blockWeekAgo: number, chainId: number): Promise<AprMap> => {
  try {
    const { farmsAtLatestBlock, farmsOneWeekAgo } = await coreClients[chainId].request<FarmsResponse>(
      gql`
...
const fetchAndUpdateLPsAPR = async () => {
  Promise.all(
    FETCH_CHAIN_ID.map(async (chainId) => {
      ...
      fs.writeFile(
        `apps/exchange/src/config/constants/lpAprs/${chainId}.json`,
